//! Parity scene `torusknot` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-torusknot.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example torusknot
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x0a0e18);
    ...
    dl.position = Vector3::new(3.0, 4.0, 5.0);
    scene.add(dl);
    let mut knot = Object3D::mesh(Mesh::new(TorusKnotGeometry::new(0.7, 0.25, 100.0, 16.0, 2.0, 3.0), StandardMaterial::new(Color::from_hex(0xcc4488)).with_roughness(0.4).with_metalness(0.0).into()));
    knot.quaternion = Euler::new(0.2, 0.5, 0.0).to_quaternion();
    scene.add(knot);
