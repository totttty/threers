//! Parity scene `render-order` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-render-order.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example render_order
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — render-order")
    ...
    scene.background = Color::from_hex(0x222233);
    let mut a = Object3D::mesh(Mesh::new(BoxGeometry::new(0.8, 0.8, 0.8), BasicMaterial::new(Color::from_hex(0xff0000)).with_opacity(0.6).with_transparent(true).into()));
    a.position.x = -0.2;
    // a.renderOrder = 1;
    scene.add(a);
    let mut b = Object3D::mesh(Mesh::new(BoxGeometry::new(0.8, 0.8, 0.8), BasicMaterial::new(Color::from_hex(0x0088ff)).with_opacity(0.6).with_transparent(true).into()));
    b.position.x = 0.2;
    // b.renderOrder = 0;
    scene.add(b);
