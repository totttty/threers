//! Parity scene `ssr` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-ssr.html`.
...
async fn run() {
    ...
    let mut dir = Object3D::light(DirectionalLight::new(Color::from_hex(0xffffff), 0.9));
    dir.position = Vector3::new(1.0, 3.0, 2.0);
    scene.add(dir);
    let mut floor = Object3D::mesh(Mesh::new(PlaneGeometry::new(6.0, 6.0), StandardMaterial::new(Color::from_hex(0x333333)).with_roughness(0.15).with_metalness(0.85).into()));
    // floor.rotation.x = -Math.PI / 2;
    floor.position.y = -0.5;
    scene.add(floor);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(0.6, 0.6, 0.6), StandardMaterial::new(Color::from_hex(0xff5533)).with_roughness(0.4).with_metalness(0.1).into()));
    cube.position = Vector3::new(0.0, 0.1, 0.0);
    scene.add(cube);
    let mut camera = PerspectiveCamera::new(45.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(0.0, 1.2, 3.0);
