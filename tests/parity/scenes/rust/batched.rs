//! Parity scene `batched` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-batched.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example batched
//! ```
...
async fn run() {
    ...
    // const batch = new THREE.BatchedMesh(9, 4096, 8192, mat);
    // const cubeG = new THREE.BoxGeometry(0.4, 0.4, 0.4);
    // const sphG  = new THREE.SphereGeometry(0.25, 12, 8);
    // const conG  = new THREE.ConeGeometry(0.25, 0.5, 8);
    // const tmp = new THREE.Matrix4();
    // const layout = [ [cubeG, 0.7], [sphG,  0.0], [conG, -0.7], ];
    // for (const [g, y] of layout) { for (let i = 0;
    // i < 3;
    // i++) { const id = batch.addGeometry(g);
    // tmp.identity().setPosition(-0.7 + i*0.7, y, 0);
    // batch.setMatrixAt(id, tmp);
    // } } scene.add(batch);
