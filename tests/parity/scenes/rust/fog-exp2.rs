//! Parity scene `fog-exp2` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-fog-exp2.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example fog_exp2
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x88aacc);
    scene.fog = FogParams { color: Color::from_hex(0x88aacc), density: 0.12, mode: 2, ..Default::default() };
    // i < 3;
    // i++) { const m = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshBasicMaterial({ color: 0xff5544 }));
    // m.position.set(0, 0, -i * 3);
    scene.add(m);
    // };
    let mut camera = PerspectiveCamera::new(45.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(0.6, 0.2, 2.0);
