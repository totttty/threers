//! Parity scene `dirlight-target` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-dirlight-target.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example dirlight_target
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — dirlight-target")
    ...
    scene.background = Color::from_hex(0x0c0c0c);
    ...
    dl.position = Vector3::new(3.0, 3.0, 0.0);
    // dl.target.position.set(-1, -1, 0);
    scene.add(dl);
    let mut sphere = Object3D::mesh(Mesh::new(SphereGeometry::new(0.8, 32, 16), StandardMaterial::new(Color::from_hex(0xddddff)).with_roughness(0.5).into()));
    scene.add(sphere);
