//! Parity scene `sphere-point` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-sphere-point.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example sphere_point
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — sphere-point")
    ...
    scene.background = Color::from_hex(0x101822);
    scene.add_light(AmbientLight::new(Color::from_hex(0xffffff), 0.15));
    let mut pl = Object3D::light(PointLight::new(Color::from_hex(0xffe0c0), 2.0).with_distance(0).with_decay(2));
    pl.position = Vector3::new(2.0, 2.5, 2.0);
    scene.add(pl);
    let mut sphere = Object3D::mesh(Mesh::new(SphereGeometry::new(1.0, 32, 16), StandardMaterial::new(Color::from_hex(0x4090ff)).with_roughness(0.35).with_metalness(0.0).into()));
    scene.add(sphere);
