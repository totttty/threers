//! Parity scene `morph` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-morph.html`.
...
async fn run() {
    ...
    // const target = new Float32Array([ -0.5, -0.5, 0,    0.5, -0.5, 0, 0.9,  0.5, 0,   -0.1,  0.5, 0 ]);
    ...
    // g.morphAttributes = { position: [new THREE.BufferAttribute(target, 3)] };
    // const mesh = new THREE.Mesh(g, new THREE.MeshBasicMaterial({ color: 0xffaa44, side: THREE.DoubleSide }));
