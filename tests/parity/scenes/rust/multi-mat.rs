//! Parity scene `multi-mat` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-multi-mat.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example multi_mat
//! ```
...
async fn run() {
    ...
    // const positions = new Float32Array([ -0.9, -0.4, 0,  -0.1, -0.4, 0,  -0.1,  0.4, 0,  -0.9,  0.4, 0, 0.1, -0.4, 0,   0.9, -0.4, 0,   0.9,  0.4, 0,   0.1,  0.4, 0, ]);
    // const indices = [0, 1, 2,  0, 2, 3,  4, 5, 6,  4, 6, 7];
    // const g = new THREE.BufferGeometry();
    // g.setAttribute('position', new THREE.BufferAttribute(positions, 3));
    // g.setAttribute('normal',   new THREE.BufferAttribute(new Float32Array(8 * 3).fill(0).map((_, i) => (i % 3 === 2) ? 1 : 0), 3));
    // g.setIndex(indices);
    // g.addGroup(0, 6, 0);
    // g.addGroup(6, 6, 1);
    // const materials = [ new THREE.MeshBasicMaterial({ color: 0xff4444, side: THREE.DoubleSide }), new THREE.MeshBasicMaterial({ color: 0x44ff44, side: THREE.DoubleSide }), ];
