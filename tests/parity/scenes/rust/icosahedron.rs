//! Parity scene `icosahedron` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-icosahedron.html`.
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — icosahedron")
    ...
    scene.background = Color::from_hex(0x0c1014);
    ...
    scene.add(dl);
    let mut ico = Object3D::mesh(Mesh::new(IcosahedronGeometry::new(1.1, 0.0), StandardMaterial::new(Color::from_hex(0x44aaff)).with_roughness(0.5).with_metalness(0.0).into()));
    ico.quaternion = Euler::new(0.3, 0.5, 0.0).to_quaternion();
    scene.add(ico);
