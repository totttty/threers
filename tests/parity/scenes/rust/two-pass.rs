//! Parity scene `two-pass` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-two-pass.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example two_pass
//! ```
...
async fn run() {
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.0, 1.0, 1.0), StandardMaterial::new(Color::from_hex(0xff8855)).into()));
    scene.add(cube);
