//! Parity scene `dodecahedron` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-dodecahedron.html`.
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — dodecahedron")
    ...
    scene.add(dl);
    scene.add(Object3D::mesh(Mesh::new(DodecahedronGeometry::new(0.9, 0.0), StandardMaterial::new(Color::from_hex(0xaa77ff)).into())));
