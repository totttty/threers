//! Parity scene `outline` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-outline.html`.
...
async fn run() {
    ...
    scene.add_light(AmbientLight::new(Color::from_hex(0xffffff), 0.7));
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(0.6, 0.6, 0.6), BasicMaterial::new(Color::from_hex(0x66aaff)).into()));
    cube.position = Vector3::new(-0.5, 0.0, 0.0);
    scene.add(cube);
    let mut sphere = Object3D::mesh(Mesh::new(SphereGeometry::new(0.35, 16, 12), BasicMaterial::new(Color::from_hex(0xff9966)).into()));
    sphere.position = Vector3::new(0.5, 0.0, 0.0);
    scene.add(sphere);
