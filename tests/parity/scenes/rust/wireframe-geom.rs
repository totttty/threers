//! Parity scene `wireframe-geom` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-wireframe-geom.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example wireframe_geom
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — wireframe-geom")
    ...
    // scene.add(new THREE.LineSegments(new THREE.WireframeGeometry(box), new THREE.LineBasicMaterial({ color: 0x44aaff })));
    let mut camera = PerspectiveCamera::new(45.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(1.0, 1.0, 2.5);
