//! Parity scene `textured-plane` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-textured-plane.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example textured_plane
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — textured-plane")
    ...
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.5, 1.5, 1.5), BasicMaterial::new(Color::from_hex(0xffffff)).into()));
    cube.quaternion = Euler::new(0.4, 0.5, 0.0).to_quaternion();
    scene.add(cube);
