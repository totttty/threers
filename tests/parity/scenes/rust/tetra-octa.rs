//! Parity scene `tetra-octa` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-tetra-octa.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example tetra_octa
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x0d0d0d);
    ...
    scene.add(dl);
    let mut tetra = Object3D::mesh(Mesh::new(TetrahedronGeometry::new(0.9, 0.0), StandardMaterial::new(Color::from_hex(0xff7755)).with_roughness(0.5).with_metalness(0.0).into()));
    tetra.position = Vector3::new(-1.0, 0.0, 0.0);
    tetra.quaternion = Euler::new(0.3, 0.5, 0.0).to_quaternion();
    scene.add(tetra);
    let mut octa = Object3D::mesh(Mesh::new(OctahedronGeometry::new(0.9, 0.0), StandardMaterial::new(Color::from_hex(0x77cc55)).with_roughness(0.4).with_metalness(0.0).into()));
    octa.position = Vector3::new(1.0, 0.0, 0.0);
    octa.quaternion = Euler::new(0.3, 0.6, 0.0).to_quaternion();
    scene.add(octa);
    let mut camera = PerspectiveCamera::new(50.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(0.0, 0.2, 4.0);
