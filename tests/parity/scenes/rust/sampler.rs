//! Parity scene `sampler` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-sampler.html`.
...
async fn run() {
    ...
    // const A = px(255, 102, 51), B = px(34, 102, 204);
    // const data = new Uint8Array([ ...A, ...B, ...A, ...B, ...B, ...A, ...B, ...A, ...A, ...B, ...A, ...B, ...B, ...A, ...B, ...A, ]);
    ...
    // g.setAttribute('position', new THREE.BufferAttribute(new Float32Array([ -0.8, -0.8, 0,  0.8, -0.8, 0,  0.8, 0.8, 0,  -0.8, 0.8, 0, ]), 3));
    // g.setAttribute('uv', new THREE.BufferAttribute(new Float32Array([ 0, 0,  3, 0,  3, 3,  0, 3, ]), 2));
