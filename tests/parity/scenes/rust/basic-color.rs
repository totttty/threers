//! Parity scene `basic-color` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-basic-color.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example basic_color
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — basic-color")
    ...
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.4, 1.4, 1.4), BasicMaterial::new(Color::from_hex(0x33cc66)).into()));
