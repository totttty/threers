//! Parity scene `geo-translate` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-geo-translate.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example geo_translate
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — geo-translate")
