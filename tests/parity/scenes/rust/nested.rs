//! Parity scene `nested` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-nested.html`.
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x202020);
    ...
    grp.quaternion = Euler::new(0.0, 0.6, 0.0).to_quaternion();
    grp.position = Vector3::new(0.0, 0.0, 0.0);
    let mut child = Object3D::mesh(Mesh::new(BoxGeometry::new(0.8, 0.8, 0.8), StandardMaterial::new(Color::from_hex(0xff8800)).with_roughness(0.4).with_metalness(0.0).into()));
    child.position = Vector3::new(0.9, 0.0, 0.0);
    child.quaternion = Euler::new(0.3, 0.0, 0.0).to_quaternion();
    // grp.add(child);
    scene.add(grp);
