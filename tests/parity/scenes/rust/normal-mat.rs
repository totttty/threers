//! Parity scene `normal-mat` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-normal-mat.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example normal_mat
//! ```
...
async fn run() {
    ...
    let mut sphere = Object3D::mesh(Mesh::new(SphereGeometry::new(1.1, 48, 24), NormalMaterial::new().into()));
    scene.add(sphere);
