//! Parity scene `cube` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-cube.html`.
...
async fn run() {
    ...
    // function showError(e) { errEl.style.display = 'block';
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.0, 1.0, 1.0), StandardMaterial::new(Color::from_hex(0xff6633)).with_roughness(0.4).with_metalness(0.1).into()));
    cube.quaternion = Euler::new(0.5, 0.8, 0.0).to_quaternion();
    scene.add(cube);
    ...
    // Wait one frame for the GPU to flush before letting puppeteer capture. await new Promise(r => requestAnimationFrame(() => r()));
