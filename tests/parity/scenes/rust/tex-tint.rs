//! Parity scene `tex-tint` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-tex-tint.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example tex_tint
//! ```
...
async fn run() {
    ...
    // const data = new Uint8Array([ 255, 255, 255, 255,    96,  96,  96, 255, 96,  96,  96, 255,   255, 255, 255, 255, ]);
    // const tex = new THREE.DataTexture(data, 2, 2);
    // tex.needsUpdate = true;
    // const mat = new THREE.MeshBasicMaterial({ map: tex, color: 0xff8844 });
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.4, 1.4, 1.4), undefined.clone()));
