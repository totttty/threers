//! Parity scene `ssao` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-ssao.html`.
...
async fn run() {
    ...
    // seedRandom(0x5a00426);
    let mut scene = Scene::new();
    scene.background = Color::from_hex(0x888888);
    scene.add_light(AmbientLight::new(Color::from_hex(0xffffff), 0.35));
    let mut dir = Object3D::light(DirectionalLight::new(Color::from_hex(0xffffff), 1.0));
    dir.position = Vector3::new(2.0, 4.0, 3.0);
    scene.add(dir);
    let mut floor = Object3D::mesh(Mesh::new(BoxGeometry::new(3.0, 0.2, 3.0), StandardMaterial::new(Color::from_hex(0xcccccc)).with_roughness(0.9).with_metalness(0.0).into()));
    floor.position.y = -0.6;
    scene.add(floor);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(0.7, 0.7, 0.7), StandardMaterial::new(Color::from_hex(0x6699ff)).with_roughness(0.6).with_metalness(0.0).into()));
    scene.add(cube);
    let mut camera = PerspectiveCamera::new(45.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(0.0, 0.5, 3.5);
