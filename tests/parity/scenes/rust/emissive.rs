//! Parity scene `emissive` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-emissive.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example emissive
//! ```
...
async fn run() {
    ...
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.2, 1.2, 1.2), StandardMaterial::new(Color::from_hex(0x222222)).with_roughness(0.5).with_metalness(0.0).into()));
    cube.quaternion = Euler::new(0.4, 0.6, 0.0).to_quaternion();
    scene.add(cube);
