//! Parity scene `reflector` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-reflector.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example reflector
//! ```

use std::sync::Arc;

use threers::cameras::Camera;
use threers::materials::MirrorMaterial;
use threers::{
    AmbientLight,
    BasicMaterial,
    BoxGeometry,
