//! Parity scene `transparent` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-transparent.html`.
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — transparent")
    ...
    let mut back = Object3D::mesh(Mesh::new(BoxGeometry::new(1.0, 1.0, 1.0), BasicMaterial::new(Color::from_hex(0xff4444)).into()));
    back.position = Vector3::new(0.0, 0.0, -0.6);
    scene.add(back);
    let mut front = Object3D::mesh(Mesh::new(BoxGeometry::new(1.4, 1.4, 1.4), BasicMaterial::new(Color::from_hex(0x44aaff)).with_opacity(0.5).with_transparent(true).into()));
    front.position = Vector3::new(0.0, 0.0, 0.4);
    front.quaternion = Euler::new(0.3, 0.4, 0.0).to_quaternion();
    scene.add(front);
