//! Parity scene `depth-overlap` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-depth-overlap.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example depth_overlap
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — depth-overlap")
    ...
    scene.add(dl);
    let mut back = Object3D::mesh(Mesh::new(BoxGeometry::new(1.5, 1.5, 1.5), StandardMaterial::new(Color::from_hex(0xcc4444)).with_roughness(0.5).with_metalness(0.0).into()));
    back.position = Vector3::new(0.3, 0.0, -0.5);
    scene.add(back);
    let mut front = Object3D::mesh(Mesh::new(BoxGeometry::new(1.0, 1.0, 1.0), StandardMaterial::new(Color::from_hex(0x44cc44)).with_roughness(0.5).with_metalness(0.0).into()));
    front.position = Vector3::new(-0.3, 0.0, 0.5);
    front.quaternion = Euler::new(0.3, 0.3, 0.0).to_quaternion();
    scene.add(front);
