//! Parity scene `parametric` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-parametric.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example parametric
//! ```
...
async fn run() {
    ...
    scene.add(dl);
    // const func = (u, v, target) => { target.set((u - 0.5) * 2, (v - 0.5) * 2, 0.4 * Math.sin(Math.PI * u) * Math.sin(Math.PI * v));
    // };
    let mut mesh = Object3D::mesh(Mesh::new(// ParametricGeometry: pass a Rust closure to ParametricGeometry::new(...), { let mut m = StandardMaterial::new(Color::from_hex(0x77aaff)).with_roughness(0.45).with_metalness(0.0); m.side = 2.into() }));
    mesh.quaternion = Euler::new(-0.2, 0.3, 0.0).to_quaternion();
    scene.add(mesh);
