//! Parity scene `hemi-light` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-hemi-light.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example hemi_light
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x181828);
    // scene.add(new THREE.HemisphereLight(0x80a0ff, 0x806040, 1.0));
    let mut sphere = Object3D::mesh(Mesh::new(SphereGeometry::new(1.1, 32, 16), StandardMaterial::new(Color::from_hex(0xffffff)).with_roughness(0.6).with_metalness(0.0).into()));
    scene.add(sphere);
