//! Parity scene `halftone-postfx` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-halftone-postfx.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example halftone_postfx
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — halftone-postfx")
    ...
    scene.add(Object3D::mesh(Mesh::new(SphereGeometry::new(0.75, 24, 16), BasicMaterial::new(Color::from_hex(0x333333)).into())));
