//! Parity scene `postfx-copy` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-postfx-copy.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example postfx_copy
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — postfx-copy")
