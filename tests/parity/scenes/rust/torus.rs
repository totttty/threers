//! Parity scene `torus` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-torus.html`.
...
async fn run() {
    ...
    scene.add(dl);
    let mut torus = Object3D::mesh(Mesh::new(TorusGeometry::new(0.8, 0.28, 16.0, 48.0), StandardMaterial::new(Color::from_hex(0xddaa44)).with_roughness(0.45).with_metalness(0.0).into()));
    torus.quaternion = Euler::new(0.5, 0.3, 0.0).to_quaternion();
    scene.add(torus);
