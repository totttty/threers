//! Parity scene `multi-shadow` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-multi-shadow.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example multi_shadow
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — multi-shadow")
