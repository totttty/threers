//! Parity scene `axes-grid` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-axes-grid.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example axes_grid
//! ```
...
async fn run() {
    ...
    scene.add(Object3D::mesh(Mesh::new(BoxGeometry::new(0.5, 0.5, 0.5), BasicMaterial::new(Color::from_hex(0x66aaff)).into())));
