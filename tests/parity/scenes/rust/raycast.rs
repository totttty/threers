//! Parity scene `raycast` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-raycast.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example raycast
//! ```
...
async fn run() {
    ...
    // g.setAttribute('position', new THREE.BufferAttribute(new Float32Array([-0.5,-0.5,0, 0.5,-0.5,0, 0,0.5,0]), 3));
    // g.setAttribute('normal',   new THREE.BufferAttribute(new Float32Array([0,0,1, 0,0,1, 0,0,1]), 3));
    // g.setIndex([0, 1, 2]);
    // const mesh = new THREE.Mesh(g, new THREE.MeshBasicMaterial({ color: 0xff8844, side: THREE.DoubleSide }));
    scene.add(mesh);
    // const raycaster = new THREE.Raycaster(new THREE.Vector3(0, 0, 2), new THREE.Vector3(0, 0, -1));
    // const hits = raycaster.intersectObject(mesh);
    // const color = hits.length > 0 ? 0x44ff44 : 0xff4444;
    let mut indicator = Object3D::mesh(Mesh::new(PlaneGeometry::new(0.4, 0.4), BasicMaterial::new(Color::from_hex(0xffffff)).into()));
    indicator.position = Vector3::new(0.0, 0.0, 1.0);
    scene.add(indicator);
