//! Parity scene `circle` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-circle.html`.
...
async fn run() {
    ...
    scene.add(dl);
    scene.add(Object3D::mesh(Mesh::new(CircleGeometry::new(0.85, 48.0), { let mut m = StandardMaterial::new(Color::from_hex(0x66aaff)); m.side = 2.into() })));
