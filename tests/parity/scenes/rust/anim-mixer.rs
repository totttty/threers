//! Parity scene `anim-mixer` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-anim-mixer.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example anim_mixer
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x1c1c22);
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(0.6, 0.6, 0.6), StandardMaterial::new(Color::from_hex(0x66aaff)).with_roughness(0.5).with_metalness(0.0).into()));
    // cube.name = 'Box';
    scene.add(cube);
    // const posTrack = new THREE.VectorKeyframeTrack('Box.position', [0, 1], [-1, 0, 0, 1, 0, 0]);
    // const clip = new THREE.AnimationClip('move', 1, [posTrack]);
    // const mixer = new THREE.AnimationMixer(scene);
    // const action = mixer.clipAction(clip);
    // action.play();
    // mixer.update(0.5);
    // if (Math.abs(cube.position.x) > 0.05) throw new Error('Cube did not move to t=0.5: x=' + cube.position.x);
