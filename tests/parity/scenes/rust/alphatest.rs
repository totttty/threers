//! Parity scene `alphatest` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-alphatest.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example alphatest
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0xffffff);
    // const tex = new THREE.DataTexture(new Uint8Array([0, 0, 0, 255, 255, 255, 255, 0, 0, 255, 255, 255, 255, 255, 255, 255]), 2, 2);
    // tex.needsUpdate = true;
    // tex.magFilter = THREE.NearestFilter;
    // tex.minFilter = THREE.NearestFilter;
    scene.add(Object3D::mesh(Mesh::new(PlaneGeometry::new(1.6, 1.6), BasicMaterial::new(Color::from_hex(0xffffff)).with_transparent(true).into())));
