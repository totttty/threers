//! Parity scene `backside` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-backside.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example backside
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x223344);
    scene.add(Object3D::mesh(Mesh::new(SphereGeometry::new(0.9, 24, 16), { let mut m = BasicMaterial::new(Color::from_hex(0xff6644)); m.side = 1.into() })));
    let mut camera = PerspectiveCamera::new(45.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(0.0, 0.0, 2.5);
