//! Parity scene `edges-geom` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-edges-geom.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example edges_geom
//! ```
...
async fn run() {
    ...
    // const wire = new THREE.LineSegments(edges, new THREE.LineBasicMaterial({ color: 0xff7733 }));
    wire.quaternion = Euler::new(0.4, 0.6, 0.0).to_quaternion();
    scene.add(wire);
