//! Parity scene `buffergeo-load` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-buffergeo-load.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example buffergeo_load
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — buffergeo-load")
    ...
    // const geom = await new Promise((resolve, reject) => loader.load('/tests/parity/scenes/triangle.json', resolve, undefined, reject));
