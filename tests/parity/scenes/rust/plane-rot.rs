//! Parity scene `plane-rot` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-plane-rot.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example plane_rot
//! ```
...
async fn run() {
    ...
    dl.position = Vector3::new(0.0, 0.0, 3.0);
    scene.add(dl);
    let mut plane = Object3D::mesh(Mesh::new(PlaneGeometry::new(2.5, 1.8), { let mut m = StandardMaterial::new(Color::from_hex(0xddaa44)).with_roughness(0.6).with_metalness(0.0); m.side = 2.into() }));
    plane.quaternion = Euler::new(-0.4, 0.3, 0.1).to_quaternion();
    scene.add(plane);
    let mut camera = PerspectiveCamera::new(40.0, 800.0 / 600.0, 0.1, 100.0);
