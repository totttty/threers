//! Parity scene `cone` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-cone.html`.
...
async fn run() {
    ...
    scene.add(dl);
    let mut cone = Object3D::mesh(Mesh::new(ConeGeometry::new(0.8, 1.6, 28.0), StandardMaterial::new(Color::from_hex(0x88ddff)).with_roughness(0.55).with_metalness(0.0).into()));
    cone.quaternion = Euler::new(0.3, 0.5, 0.0).to_quaternion();
    scene.add(cone);
