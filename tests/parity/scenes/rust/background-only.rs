//! Parity scene `background-only` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-background-only.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example background_only
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — background-only")
    ...
    scene.background = Color::from_hex(0x335577);
