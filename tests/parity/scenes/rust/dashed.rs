//! Parity scene `dashed` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-dashed.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example dashed
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x0a0c10);
    // const g = new THREE.BufferGeometry();
    // g.setAttribute('position', new THREE.BufferAttribute(new Float32Array([ -0.9, -0.3, 0,    0.9, -0.3, 0, -0.9,  0.3, 0,    0.9,  0.3, 0, ]), 3));
    // const mat = new THREE.LineDashedMaterial({ color: 0xffaa44, dashSize: 0.15, gapSize: 0.08 });
