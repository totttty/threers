//! Parity scene `shadow-mat` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-shadow-mat.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example shadow_mat
//! ```
...
async fn run() {
    ...
    scene.add(floor);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(0.6, 0.6, 0.6), StandardMaterial::new(Color::from_hex(0x66aaff)).into()));
    cube.position.y = 0.0;
    cube.cast_shadow = true;
    scene.add(cube);
    let mut dl = Object3D::light(DirectionalLight::new(Color::from_hex(0xffffff), 1.2));
    dl.position = Vector3::new(2.0, 4.0, 2.0);
    dl.cast_shadow = true;
