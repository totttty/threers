//! Parity scene `ring` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-ring.html`.
...
async fn run() {
    ...
    dl.position = Vector3::new(2.0, 2.0, 3.0);
    scene.add(dl);
    scene.add(Object3D::mesh(Mesh::new(RingGeometry::new(0.35, 0.85, 48.0), { let mut m = StandardMaterial::new(Color::from_hex(0xff8844)); m.side = 2.into() })));
