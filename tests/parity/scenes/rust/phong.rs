//! Parity scene `phong` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-phong.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example phong
//! ```
...
async fn run() {
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.2, 1.2, 1.2), PhongMaterial::new(Color::from_hex(0x6688cc)).into()));
