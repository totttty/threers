//! Parity scene `lineloop` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-lineloop.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example lineloop
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x0a0c0e);
    // const pts = [ -0.7, -0.5, 0, 0.7, -0.5, 0, 0.7,  0.5, 0, -0.7,  0.5, 0 ];
    // const g = new THREE.BufferGeometry();
    // g.setAttribute('position', new THREE.BufferAttribute(new Float32Array(pts), 3));
    // const loop = new THREE.LineLoop(g, new THREE.LineBasicMaterial({ color: 0x44ff88 }));
    scene.add(loop);
