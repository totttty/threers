//! Parity scene `mesh-scale` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-mesh-scale.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example mesh_scale
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x1c1c1c);
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.0, 1.0, 1.0), StandardMaterial::new(Color::from_hex(0xff8855)).with_roughness(0.5).with_metalness(0.0).into()));
    cube.scale = Vector3::new(2.5, 0.6, 1.0);
    cube.quaternion = Euler::new(0.3, 0.5, 0.0).to_quaternion();
    scene.add(cube);
