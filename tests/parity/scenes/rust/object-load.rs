//! Parity scene `object-load` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-object-load.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example object_load
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — object-load")
    ...
    // const scene = await new Promise((resolve, reject) => loader.load('/tests/parity/scenes/object-scene.json', resolve, undefined, reject));
    // if (!scene.background) scene.background = new THREE.Color(0x202428);
    let mut camera = PerspectiveCamera::new(45.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(0.5, 0.5, 2.0);
