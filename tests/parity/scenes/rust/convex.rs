//! Parity scene `convex` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-convex.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example convex
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x161618);
    scene.add_light(AmbientLight::new(Color::from_hex(0xffffff), 0.5));
    let mut dl = Object3D::light(DirectionalLight::new(Color::from_hex(0xffffff), 0.7));
    dl.position = Vector3::new(2.0, 3.0, 4.0);
    scene.add(dl);
    // const pts = [ new THREE.Vector3( 1,  0,  0), new THREE.Vector3(-1,  0,  0), new THREE.Vector3( 0,  1,  0), new THREE.Vector3( 0, -1,  0), new THREE.Vector3( 0,  0,  1), new THREE.Vector3( 0,  0, -1), ];
    // const g = new THREE.ConvexGeometry(pts);
    // const mesh = new THREE.Mesh(g, new THREE.MeshStandardMaterial({ color: 0x88ddff, roughness: 0.4 }));
    mesh.quaternion = Euler::new(0.3, 0.5, 0.0).to_quaternion();
    scene.add(mesh);
