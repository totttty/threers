//! Parity scene `quat-track` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-quat-track.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example quat_track
//! ```
...
async fn run() {
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(0.7, 0.7, 0.7), StandardMaterial::new(Color::from_hex(0x66aaff)).into()));
    // cube.name = 'Box';
    scene.add(cube);
