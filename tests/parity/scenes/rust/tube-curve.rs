//! Parity scene `tube-curve` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-tube-curve.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example tube_curve
//! ```
...
async fn run() {
    ...
    scene.add(dl);
    // const curve = new THREE.CatmullRomCurve3([ new THREE.Vector3(-1, -0.5, 0), new THREE.Vector3(-0.5, 0.5, 0), new THREE.Vector3(0.5, -0.5, 0), new THREE.Vector3(1, 0.5, 0),;
    // ]);
    scene.add(Object3D::mesh(Mesh::new(TubeGeometry::new(curve, 32.0, 0.15, 8.0, false), StandardMaterial::new(Color::from_hex(0xff9966)).into())));
