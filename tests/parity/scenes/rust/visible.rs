//! Parity scene `visible` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-visible.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example visible
//! ```
...
async fn run() {
    ...
    let mut hidden = Object3D::mesh(Mesh::new(BoxGeometry::new(1.0, 1.0, 1.0), BasicMaterial::new(Color::from_hex(0xff0000)).into()));
    // hidden.visible = false;
    hidden.position = Vector3::new(-0.6, 0.0, 0.0);
    scene.add(hidden);
    let mut shown = Object3D::mesh(Mesh::new(BoxGeometry::new(1.0, 1.0, 1.0), BasicMaterial::new(Color::from_hex(0x00ff00)).into()));
    shown.position = Vector3::new(0.6, 0.0, 0.0);
    scene.add(shown);
