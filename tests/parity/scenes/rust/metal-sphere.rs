//! Parity scene `metal-sphere` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-metal-sphere.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example metal_sphere
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — metal-sphere")
    ...
    scene.background = Color::from_hex(0x080808);
    ...
    scene.add(dl);
    let mut sphere = Object3D::mesh(Mesh::new(SphereGeometry::new(1.0, 64, 32), StandardMaterial::new(Color::from_hex(0xc0c0c0)).with_roughness(0.15).with_metalness(1.0).into()));
    scene.add(sphere);
