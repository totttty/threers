//! Parity scene `extrude` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-extrude.html`.
...
async fn run() {
    ...
    let mut dl = Object3D::light(DirectionalLight::new(Color::from_hex(0xffffff), 0.9));
    ...
    // shape.lineTo( 0.5,  0.3);
    // shape.lineTo(-0.5,  0.3);
    // const opts = { depth: 0.4, bevelEnabled: false, curveSegments: 1 };
    let mut mesh = Object3D::mesh(Mesh::new(ExtrudeGeometry::new(shape, opts), StandardMaterial::new(Color::from_hex(0x88ddff)).with_roughness(0.5).into()));
    mesh.quaternion = Euler::new(-0.4, 0.7, 0.0).to_quaternion();
    mesh.position.z = -0.2;
    scene.add(mesh);
    let mut camera = PerspectiveCamera::new(45.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(0.0, 0.2, 2.6);
