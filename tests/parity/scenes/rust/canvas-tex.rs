//! Parity scene `canvas-tex` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-canvas-tex.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example canvas_tex
//! ```
...
async fn run() {
    ...
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.4, 1.4, 1.4), BasicMaterial::new(Color::from_hex(0xffffff)).into()));
