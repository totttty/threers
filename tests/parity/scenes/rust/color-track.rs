//! Parity scene `color-track` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-color-track.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example color_track
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — color-track")
    ...
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(0.9, 0.9, 0.9), undefined.clone()));
    // cube.name = 'Box';
    scene.add(cube);
