//! Parity scene `points` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-points.html`.
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x000010);
    // const positions = [];
    // y <= 2;
    // y++) for (let x = -2;
    // x <= 2;
    // x++) { positions.push(x * 0.4, y * 0.4, 0);
    // } const geom = new THREE.BufferGeometry();
    // geom.setAttribute('position', new THREE.BufferAttribute(new Float32Array(positions), 3));
    // const pts = new THREE.Points(geom, new THREE.PointsMaterial({ color: 0xffff00, size: 12, sizeAttenuation: false }));
    scene.add(pts);
