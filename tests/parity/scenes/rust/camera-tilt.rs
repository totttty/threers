//! Parity scene `camera-tilt` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-camera-tilt.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example camera_tilt
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — camera-tilt")
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.2, 1.2, 1.2), StandardMaterial::new(Color::from_hex(0xff8866)).with_roughness(0.4).with_metalness(0.0).into()));
    scene.add(cube);
    let mut camera = PerspectiveCamera::new(50.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(2.5, 2.5, 2.5);
