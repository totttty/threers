//! Parity scene `quaternion` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-quaternion.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example quaternion
//! ```
...
async fn run() {
    ...
    scene.background = Color::from_hex(0x181c20);
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.2, 1.2, 1.2), StandardMaterial::new(Color::from_hex(0x44ddff)).with_roughness(0.5).into()));
    // const q = new THREE.Quaternion();
    // q.setFromAxisAngle(new THREE.Vector3(1, 1, 0).normalize(), Math.PI / 4);
