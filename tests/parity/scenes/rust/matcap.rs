//! Parity scene `matcap` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-matcap.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example matcap
//! ```
...
async fn run() {
    ...
    // y++) for (let x = 0;
    // x < size;
    // x++) { const cx = x - size / 2, cy = y - size / 2;
    ...
    let mut sphere = Object3D::mesh(Mesh::new(SphereGeometry::new(0.9, 32, 16), undefined.clone()));
