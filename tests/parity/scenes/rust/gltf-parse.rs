//! Parity scene `gltf-parse` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-gltf-parse.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example gltf_parse
//! ```
...
async fn run() {
    ...
    // Same inline GLTF as the three.js side. const positions = new Float32Array([0,0,0, 1,0,0, 0,1,0]);
    ...
    // const dataURI = 'data:application/octet-stream;
    // base64,' + btoa(String.fromCharCode(...bin));
    // const gltf = { asset: { version: '2.0' }, scene: 0, scenes: [{ nodes: [0] }], nodes: [{ mesh: 0, name: 'Tri' }], meshes: [{ primitives: [{ attributes: { POSITION: 0 }, indices: 1, material: 0 }] }], materials: [{ pbrMetallicRoughness: { baseColorFactor: [0.8, 0.4, 0.2, 1.0], roughnessFactor: 0.5, metallicFactor: 0 } }], accessors: [ { bufferView: 0, componentType: 5126, count: 3, type: 'VEC3' }, { bufferView: 1, componentType: 5125, count: 3, type: 'SCALAR' }, ], bufferViews: [ { buffer: 0, byteOffset: 0, byteLength: 36 }, { buffer: 0, byteOffset: 36, byteLength: 12 }, ], buffers: [{ uri: dataURI, byteLength: 48 }], };
    // const loader = new THREE.GLTFLoader();
    // Replicate the GLTF buffer resolution that .load() does, but pass the buffers directly. const buffers = [bin.buffer];
    // const data = loader.parse(gltf, buffers);
    // if (!data.scene) throw new Error('No scene returned');
    ...
    camera.position = Vector3::new(0.4, 0.4, 2.0);
    camera.look_at(Vector3::new(0.4, 0.4, 0.0));
