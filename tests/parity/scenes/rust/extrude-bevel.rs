//! Parity scene `extrude-bevel` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-extrude-bevel.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example extrude_bevel
//! ```
...
async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("threers — extrude-bevel")
    ...
    // shape.lineTo(-0.5, 0.5);
    // shape.closePath();
    scene.add(Object3D::mesh(Mesh::new(ExtrudeGeometry::new(shape, { depth: 0.35, bevelEnabled: true, bevelSize: 0.08, bevelThickness: 0.08, bevelSegments: 2 }), StandardMaterial::new(Color::from_hex(0x66ccaa)).into())));
    let mut camera = PerspectiveCamera::new(45.0, 800.0 / 600.0, 0.1, 100.0);
    camera.position = Vector3::new(1.2, 1.0, 2.8);
