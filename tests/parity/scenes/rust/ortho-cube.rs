//! Parity scene `ortho-cube` — native Rust (winit + wgpu).
//!
//! Generated from `tests/parity/scenes/threers-ortho-cube.html`.
//! Compare with the JavaScript tab above; adjust imports before copying to `examples/`.
//!
//! ```text
//! cargo run --example ortho_cube
//! ```
...
async fn run() {
    ...
    scene.add(dl);
    let mut cube = Object3D::mesh(Mesh::new(BoxGeometry::new(1.2, 1.2, 1.2), StandardMaterial::new(Color::from_hex(0xff44aa)).with_roughness(0.5).with_metalness(0.0).into()));
    cube.quaternion = Euler::new(0.4, 0.7, 0.1).to_quaternion();
    scene.add(cube);
    // const aspect = 800/600;
    // const viewSize = 2.5;
    // const cam = new THREE.OrthographicCamera(-viewSize*aspect, viewSize*aspect, viewSize, -viewSize, 0.1, 100);
    camera.position = Vector3::new(2.0, 2.0, 3.0);
