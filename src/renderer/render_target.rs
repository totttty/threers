...
impl RenderTarget {
    ...
    pub fn new_cube(device: &Arc<wgpu::Device>, side: u32, format: wgpu::TextureFormat) -> CubeRenderTarget {
        let size = wgpu::Extent3d { width: side.max(1), height: side.max(1), depth_or_array_layers: 6 };
        let srgb_variant = format.add_srgb_suffix();
        let extra_formats = if srgb_variant != format { vec![srgb_variant] } else { vec![] };
        ...
        CubeRenderTarget { side, color_texture, face_views, cube_view, depth_texture, depth_view, format }
    }

    pub fn new(device: &Arc<wgpu::Device>, width: u32, height: u32, format: wgpu::TextureFormat) -> Self {
        ...
        Self { width, height, color_texture, color_view, depth_texture, depth_view, format }
    }
}
