...
impl ExrLoader {
    pub fn parse(bytes: &[u8]) -> Result<Texture, ExrError> {
        if bytes.len() < 8 || bytes[0..4] != MAGIC { return Err(ExrError::BadMagic); }
        ...
        loop {
            if pos >= bytes.len() { return Err(ExrError::BadHeader); }
            ...
            pos += 1;
            // Size.
            if pos + 4 > bytes.len() { return Err(ExrError::BadHeader); }
            let size = u32::from_le_bytes([bytes[pos], bytes[pos+1], bytes[pos+2], bytes[pos+3]]) as usize;
            pos += 4;
            if pos + size > bytes.len() { return Err(ExrError::BadHeader); }
            let data = &bytes[pos..pos + size];
            pos += size;
            match name.as_str() {
                "dataWindow" if attr_type == "box2i" && size >= 16 => {
                    let read_i = |o| i32::from_le_bytes([data[o], data[o+1], data[o+2], data[o+3]]);
        ...
        let Some(dw) = data_window else { return Err(ExrError::BadHeader); };
        let width = (dw[2] - dw[0] + 1).max(0) as usize;
        let height = (dw[3] - dw[1] + 1).max(0) as usize;
        if width == 0 || height == 0 { return Err(ExrError::BadHeader); }

        // Determine RGB(A) channel layout.
        let pixel_type_of = |name: &str| -> Option<u8> {
            channels_present.iter().find(|(n, _)| n == name).map(|(_, t)| *t)
        };
        ...
        if pos + offsets_size > bytes.len() { return Err(ExrError::BadHeader); }
        pos += offsets_size;

        // For each scanline: y(i32) + size(u32) + interleaved channel data.
        let sample_size = |t: u8| -> usize { match t { 1 => 2, 2 => 4, _ => 0 } };
        ...
        let mut y = 0usize;
        while y < height {
            if pos + 8 > bytes.len() { return Err(ExrError::BadHeader); }
            let _scanline_y = i32::from_le_bytes([bytes[pos], bytes[pos+1], bytes[pos+2], bytes[pos+3]]);
            let block_size = u32::from_le_bytes([bytes[pos+4], bytes[pos+5], bytes[pos+6], bytes[pos+7]]) as usize;
            pos += 8;
            if pos + block_size > bytes.len() { return Err(ExrError::BadHeader); }
            ...
            let bytes_per_scanline: usize = sorted_chans.iter().map(|(_, t)| sample_size(*t) * width).sum();
            for s in 0..scanline_count {
                if (s + 1) * bytes_per_scanline > block.len() { break; }
                let scanline = &block[s * bytes_per_scanline..(s + 1) * bytes_per_scanline];

                let mut channel_offsets: std::collections::HashMap<&str, usize> = std::collections::HashMap::new();
                ...
                for x in 0..width {
                    let r = read_sample(r_type, scanline, channel_offsets["R"] + x * sample_size(r_type));
                    let g = read_sample(g_type, scanline, channel_offsets["G"] + x * sample_size(g_type));
                    let b = read_sample(b_type, scanline, channel_offsets["B"] + x * sample_size(b_type));
        ...
        Ok(Texture::new(width as u32, height as u32, TextureFormat::Rgba8Unorm, rgba))
    }
}
