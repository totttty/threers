...
impl HdrLoader {
    pub fn parse(bytes: &[u8]) -> Result<Texture, HdrError> {
        ...
        let dim_start = pos;
        while pos < bytes.len() && bytes[pos] != b'\n' { pos += 1; }
        let dim_line = std::str::from_utf8(&bytes[dim_start..pos]).map_err(|_| HdrError::BadHeader)?;
        pos += 1;
        let mut height = 0usize;
        let mut width = 0usize;
        for tok in dim_line.split_whitespace() {
            if let Ok(n) = tok.parse::<usize>() {
                if height == 0 { height = n; } else { width = n; }
            }
        }
        if width == 0 || height == 0 { return Err(HdrError::BadHeader); }

        // Decode scanlines. Each scanline is `width` pixels of RGBE.
        let mut rgba = Vec::with_capacity(width * height * 4);
        for _ in 0..height {
            if pos + 4 > bytes.len() { return Err(HdrError::BadScanline); }
            let r = bytes[pos];
            let g = bytes[pos + 1];
            let b1 = bytes[pos + 2];
            let b2 = bytes[pos + 3];
            ...
            if r == 2 && g == 2 && b1 < 128 {
                ...
                pos += 4;
                if scan_w != width { return Err(HdrError::BadScanline); }
                let mut channels: [Vec<u8>; 4] = [
                    vec![0u8; width], vec![0u8; width], vec![0u8; width], vec![0u8; width],
                ];
                for c in &mut channels {
                    let mut x = 0usize;
                    while x < width {
                        if pos >= bytes.len() { return Err(HdrError::BadScanline); }
                        let n = bytes[pos];
                        pos += 1;
                        if n > 128 {
                            // Run-length
                ...
                for x in 0..width {
                    let rgb = rgbe_to_rgb(channels[0][x], channels[1][x], channels[2][x], channels[3][x]);
                ...
                for _ in 1..width {
                    if pos + 4 > bytes.len() { return Err(HdrError::BadScanline); }
                    let rgb = rgbe_to_rgb(bytes[pos], bytes[pos+1], bytes[pos+2], bytes[pos+3]);
                    rgba.extend_from_slice(&rgb);
                    pos += 4;
                }
            }
