...
impl PlyLoader {
    pub fn parse(bytes: &[u8]) -> BufferGeometry {
        ...
        for (i, &b) in bytes.iter().enumerate() {
            if b == b'\n' {
                let line = std::str::from_utf8(&bytes[last_nl..i]).unwrap_or("").trim_end_matches('\r').to_string();
        ...
        if header_end == 0 { return BufferGeometry::new(); }
            ...
            match parts.as_slice() {
                ...
                ["element", name, count] => {
                    if let Ok(n) = count.parse::<usize>() {
                        elements.push(Element { name: (*name).into(), count: n, props: Vec::new() });
                    }
                }
                ["property", "list", count_ty, item_ty, name] => {
                    ...
                    if let Some(e) = elements.last_mut() {
                        e.props.push(Prop { name: (*name).into(), scalar: None, list: Some((count_ty, item_ty)) });
                    }
                }
                ["property", ty, name] => {
                    let ty: &'static str = leak(ty);
                    if let Some(e) = elements.last_mut() {
                        e.props.push(Prop { name: (*name).into(), scalar: Some(ty), list: None });
                    }
                }
                _ => {}
            }
        ...
        if format == "ascii" {
            ...
            for elem in &elements {
                for _ in 0..elem.count {
                    let line = match lines.next() { Some(l) => l, None => break };
                    ...
                    let mut r = 0.0; let mut gr = 0.0; let mut bl = 0.0;
                    let mut have_n = false;
                    let mut have_uv = false;
                    let mut have_c = false;
                    if elem.name == "vertex" {
                        for prop in &elem.props {
                            if prop.list.is_some() { ti += 1; continue; }
                            let tok = toks.get(ti).copied().unwrap_or("0");
                            let val: f32 = tok.parse().unwrap_or(0.0);
                            match prop.name.as_str() {
                                "x" => px = val, "y" => py = val, "z" => pz = val,
                                "nx" => { nx = val; have_n = true; }
                                "ny" => { ny = val; have_n = true; }
                                "nz" => { nz = val; have_n = true; }
                                "s" | "u" | "texture_u" => { u = val; have_uv = true; }
                                "t" | "v" | "texture_v" => { v = val; have_uv = true; }
                                "red" => { r = val / 255.0; have_c = true; }
                                "green" => { gr = val / 255.0; have_c = true; }
                                "blue" => { bl = val / 255.0; have_c = true; }
                                _ => {}
                            }
                            ti += 1;
                        }
                        positions.extend_from_slice(&[px, py, pz]);
                        if have_n { normals.extend_from_slice(&[nx, ny, nz]); }
                        if have_uv { uvs.extend_from_slice(&[u, v]); }
                        if have_c { colors.extend_from_slice(&[r, gr, bl]); }
                    } else if elem.name == "face" {
                        for prop in &elem.props {
                            if let Some((_, _)) = prop.list {
                                let count: u32 = toks.get(ti).and_then(|s| s.parse().ok()).unwrap_or(0);
                                ti += 1;
                                let mut idx: Vec<u32> = Vec::with_capacity(count as usize);
                                for _ in 0..count {
                                    let v: u32 = toks.get(ti).and_then(|s| s.parse().ok()).unwrap_or(0);
                                    idx.push(v);
                                    ti += 1;
                                }
        ...
        if !colors.is_empty() { g.set_attribute("color", BufferAttribute::new(colors, 3)); }
        if !indices.is_empty() { g.set_index(indices); }
        g
    }
}
