import { Ray, Matrix4, Vector3, Vector4, Triangle, Line3 } from '/web/threejs-shim.js';
...
export function getHitSideWithCoplanarCheck( tri, bvh ) {
	...
	// Deterministic jitter (matches native `src/csg/hit_side.rs` for parity exports).
	const jitters = [
		[ 0.25, -0.5, 0.5 ],
		[ -0.5, 0.25, -0.25 ],
		[ 0.5, 0.5, -0.5 ],
	...
	for ( let i = 0; i < total; i ++ ) {
		...
		_ray.direction.copy( _normal );
		_ray.direction.x += jitters[ i ][ 0 ] * JITTER_EPSILON;
		_ray.direction.y += jitters[ i ][ 1 ] * JITTER_EPSILON;
		_ray.direction.z += jitters[ i ][ 2 ] * JITTER_EPSILON;
